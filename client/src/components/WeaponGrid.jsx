...
const WeaponGrid = ({ currentTurn: initialCurrentTurn, onUpdate, turnAction: initialTurnAction, user, availableWeapons, onBanPick, bans, picks }) => {
  const [error, setError] = useState('');
  const [selectedWeapon, setSelectedWeapon] = useState(null);
  const [currentTurn, setCurrentTurn] = useState(initialCurrentTurn);
  const [turnAction, setTurnAction] = useState(initialTurnAction);

  useEffect(() => {
    setCurrentTurn(initialCurrentTurn);
    setTurnAction(initialTurnAction);
    console.log('WeaponGrid updated:', { currentTurn: initialCurrentTurn, turnAction: initialTurnAction });
  }, [initialCurrentTurn, initialTurnAction]);
  ...
  const handleWeaponClick = (weaponId) => {
    ...
    if (!turnAction || !currentTurn) {
      setError('Phiên ban/pick chưa bắt đầu');
      return;
    }
    if (!isPlayerTurn) {
  ...
  const handleLockIn = async (e, weaponId) => {
    e.preventDefault();
    if (!isPlayerTurn) {
      setError('Không phải lượt của bạn');
      return;
    ...
    try {
      ...
      await onBanPick(weaponId, turnAction);
      setSelectedWeapon(null);
      onUpdate();
    } catch (error) {
      console.error('Error in lockIn:', error);
      setError(error.response?.data?.message || 'Lỗi khi khóa lựa chọn');
    }
  };
            ...
            <Col key={weapon.id} xs={4} className="mb-2">
              ...
              >
                ...
                {isSelected && isPlayerTurn && turnAction && (
                  <button className="btn-lock" onClick={(e) => handleLockIn(e, weapon.id)}>
                    Khóa chọn
                  </button>
                )}
              </div>
