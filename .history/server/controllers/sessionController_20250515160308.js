...
const login = async (req, res) => {
  ...
  if (session && session.players.length >= 2 && !session.players.some(p => p.userId.toString() === user._id.toString())) {
...
const startTimer = (session, io, action, team) => {
      ...
      if (availableWeapons.length > 0) {
        ...
        if (action === 'ban') {
          ...
          if (session.bans.length < session.banCount * 2) {
            ...
            session.currentTurn = session.firstTurn; // Start picks with firstTurn
          }
        } else if (action === 'pick') {
        ...
        // Save session once after auto-select
        await session.save();
        io.emit('sessionUpdate', session);

        // Start timer for next turn if session is not completed
...
const coinFlip = async (req, res) => {
  ...
  try {
    ...
    session.readyStatus.player2Ready = false; // Reset ready status
    await session.save();
...
const resetSession = async (req, res) => {
  ...
  try {
    ...
    const io = req.app.get('io');
    io.emit('sessionUpdate', updatedSession);
    res.json({ message: 'Session reset', session: updatedSession });
  } catch (error) {
