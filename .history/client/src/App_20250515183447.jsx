...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on('sessionUpdate', (session) => {
      ...
      if (!session.currentTurn || session.isCompleted) {
        setTimer(null);
        setCurrentAction(null);
      }
    });
  ...
  const handleBanPick = async (weaponId, action) => {
    if (sessionData?.isCompleted) {
      setError('Phiên đã hoàn tất, không thể thực hiện hành động');
      return;
    }
    try {
      const token = localStorage.getItem('token');
      ...
      <Container fluid className="p-0 main-content">
        ...
        {sessionData?.isCompleted && (
          <Alert variant="success" className="mt-2">
            Phiên đã hoàn tất! Vui lòng reset để bắt đầu phiên mới.
          </Alert>
        )}
        {user === 'player1' && (
        ...
        <Row className="main-content">
          ...
          <Col xs={12} md={6} className="p-3 weapon-grid">
            <div className="timer-display">
              {timer !== null && currentTurn && !sessionData?.isCompleted && (
                <h3>
          ...
          <Col xs={12} md={3} className="p-3 d-flex flex-column align-items-center">
            ...
            {locked && !sessionData?.isCompleted && (
