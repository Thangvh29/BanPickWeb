...
function App() {
  ...
  const fetchSessionData = useCallback(async () => {
    ...
    try {
      ...
      if (!newData || Object.keys(newData).length === 0) {
        console.log('Session data empty, initializing default');
      ...
      if (!error.response) {
        ...
        setError('Lỗi khi tải dữ liệu phiên');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    socket.on('sessionUpdate', (session) => {
      ...
      setSessionData(prev => {
        if (JSON.stringify(prev) !== JSON.stringify(session)) {
          console.log('Session data updated via socket:', session);
          return session;
        }
        return prev;
      });
    ...
    socket.on('coinFlip', ({ firstTurn, coinFace }) => {
      console.log('Received coinFlip:', { firstTurn, coinFace });
  ...
  const handleLock = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      console.log('Locking ban/pick counts:', { banCount, pickCount });
      await axios.post(
        `${API_URL}/update`,
  ...
  const handleReady = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      console.log('Initiating coin flip');
      await axios.post(
        `${API_URL}/coinflip`,
        { selectedWeapons },
        { headers: { Authorization: `Bearer ${token}` }, timeout: 5000 }
  ...
  const handleBanPick = async (weaponId, action) => {
    try {
      const token = localStorage.getItem('token');
      console.log('Ban/pick:', { weaponId, action });
      await axios.post(
        `${API_URL}/lockin`,
        { weaponId, action },
        { headers: { Authorization: `Bearer ${token}` }, timeout: 5000 }
      ...
      console.error('Ban/pick error:', error);
  ...
  const handleReset = async () => {
    if (user !== 'player1') {
      setError('Chỉ Player 1 có thể reset');
      return;
    }
    try {
      const token = localStorage.getItem('token');
