...
function App() {
  ...
  const handleWeaponSelect = useCallback(async (weaponId) => {
    ...
    try {
      ...
      // Không gọi fetchSessionData, chờ sessionUpdate từ Socket.IO
      setError('');
      // Cập nhật cục bộ selectedWeapons (nếu cần, nhưng backend sẽ đồng bộ qua sessionUpdate)
      // setSelectedWeapons(prev => [...prev, weaponId]); // Tạm thời bỏ qua, đợi backend
    } catch (error) {
