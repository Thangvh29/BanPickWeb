  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/login`,
        { username, password },
        { timeout: 5000 } // Timeout 5s để tránh chờ lâu
      );
      const { token, role } = response.data;

      if (!token || !role) {
        setLocalError('Đăng nhập thất bại: Phản hồi từ server không hợp lệ');
        setIsLoading(false);
        return;
      }
      ...
      if (axios.isAxiosError(error) && error.code === 'ECONNABORTED') {
        setLocalError('Yêu cầu hết thời gian, vui lòng kiểm tra kết nối mạng');
      } else if (!error.response) {
        setLocalError('Không thể kết nối đến server, vui lòng thử lại');
      } else if (error.response.status === 400) {
        setLocalError(error.response.data.message || 'Tên người dùng hoặc mật khẩu không đúng');
      } else if (error.response.status === 500) {
        setLocalError('Lỗi server, vui lòng liên hệ quản trị viên');
      } else {
