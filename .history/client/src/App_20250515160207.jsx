...
function App() {
  ...
  const handlePrepare = async () => {
    try {
      const token = localStorage.getItem('token');
      console.log(`Player ${user} preparing`);
      await axios.post(
        `${API_URL}/ready`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
  ...
  const handleReady = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      console.log('Sending coinflip request with weapons:', selectedWeapons);
  ...
  const requiredWeapons = (parseInt(banCount) || 0) * 2 + (parseInt(pickCount) || 0) * 2;
  const isWeaponsSelected = selectedWeapons.length >= requiredWeapons && requiredWeapons > 0;
  ...
  console.log('Debug isControlDisabled:', {
    started,
    isCompleted: sessionData?.isCompleted,
    isControlDisabled
  });

  console.log('Debug isReadyDisabled:', {
    user,
    isWeaponsSelected,
    isControlDisabled,
          ...
          <Col xs={12} md={3} className="p-3 d-flex flex-column align-items-center">
            ...
            {locked && isWeaponsSelected && (
              <div className="mt-3">
                ...
                <p>Player 1: {sessionData?.readyStatus?.player1Ready ? 'Sẵn sàng' : 'Chưa sẵn sàng'}</p>
                <p>Player 2: {sessionData?.readyStatus?.player2Ready ? 'Sẵn sàng' : 'Chưa sẵn sàng'}</p>
                <Button
